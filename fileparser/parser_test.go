...
func TestEchoParser(t *testing.T) {
	...
	ep := fileparser.EchoParser{Writer: w}
...
func TestParse(t *testing.T) {
	var np fileparser.NullParser
	fpNull := fileparser.New("intro", np)
	...
	for _, tc := range testCases {
		...
		if fpNull.Stats().FilesVisited() != tc.expectedFileCount {
			t.Error("Parse(", tc.filename, ") failed - expected: ",
				tc.expectedFileCount, " files visited, got: ", fpNull.Stats().FilesVisited())
		}
		if fpNull.Stats().LinesRead() != tc.expectedLineCount {
			t.Error("Parse(", tc.filename, ") failed - expected: ",
				tc.expectedLineCount, " lines read, got: ", fpNull.Stats().LinesRead())
		}
		if fpNull.Stats().LinesParsed() != tc.expectedParsedCount {
			t.Error("Parse(", tc.filename, ") failed - expected: ",
				tc.expectedParsedCount, " lines parsed, got: ", fpNull.Stats().LinesParsed())
		}
	}
}

func TestStats(t *testing.T) {
	var s fileparser.Stats

	if fv := s.FilesVisited(); fv != 0 {
		t.Error("an empty Stats structure should have filesVisited: 0, has: ", fv)
	}
	if lr := s.LinesRead(); lr != 0 {
		t.Error("an empty Stats structure should have linesRead: 0, has: ", lr)
	}
	if lp := s.LinesParsed(); lp != 0 {
		t.Error("an empty Stats structure should have linesParsed: 0, has: ", lp)
	}
	expectedStr := "files:   0 lines read:     0 parsed:     0"
	if s := s.String(); s != expectedStr {
		t.Error("an empty Stats structure should have a String representation of: ",
			expectedStr, " has: ", s)
	}
}
