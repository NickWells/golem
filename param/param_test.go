...
func TestParamParse1(t *testing.T) {
	...
	errMap, panicked, panicVal :=
		panicSafeTestParse(ps, []string{"-param1", "99"})

	if !ps.AreSet() {
		t.Errorf("%s: params have been set but AreSet() says they haven't",
			testName)
	}

	if len(errMap) != 0 {
		t.Errorf("%s: unexpected errors were detected while parsing\n",
			testName)
		...
		t.Errorf("%s: p1 (%d) was expected to be == 99",
			testName, p1)
	} else if p2 != 0 {
		t.Errorf("%s: p2 (%d) was expected to be == 0",
			testName, p2)
	} else if ps.ProgName() != param.DfltProgName {
		t.Errorf("%s: ps.ProgName() ('%s') was expected to be == '%s'",
			testName, ps.ProgName(), param.DfltProgName)
	}
	...
	testName = "1: param.Parse - bad second parse attempt"
	errMap, panicked, panicVal = panicSafeTestParse(ps,
		[]string{"-param1", "99"})
	if panicked {
		...
		errMapCheck(t, testName, errMap, map[string][]string{
			"": []string{"param.Parse has already been called"},
		})
	}
}
