...
// MakeActionFunc returns a function suitable for passing to the
// AddPostAction method of a param.ByName object. This uses a pre-existing
// SetOnce value and so it could be set on multiple different parameters but
// the expected use is to have a separate SetOnce value for each parameter
// that you want to protect
func (so *SetOnce) MakeActionFunc(action SetOnceErrAction) param.ActionFunc {
	return func(source string, loc location.L, p *param.ByName, paramVals []string) error {
